...
int main(int argc, char *argv[]){
	...
	char *filename = "patient1.txt";
	char *patientinfo = readpi(filename);
	if(!patientinfo){
		fprintf(stderr, "Error in reading info of patient1.\n");
		return 3;
	}
	...
	char *p1insurfile = "patient1insurance.txt";
	char *p1insur = (char *)malloc(MAXDATALEN * sizeof(char));
	memset(p1insur, 0, (MAXDATALEN * sizeof(char)));
	readinsurinfo(p1insur, p1insurfile);
...
buf[numbytes] = '\0';
printf("The following appointments are available for Patient 1:\n");
printf("%s\n", buf);
int avaindex[6];
int i;
for(i = 0; i < 6; i++){
...
if(strcmp(buf, "notavailable")){
	...
	printf("The requested appointment from Patient 1 is not available. Exiting...\n\n");
	exit(0);
}

close(sockfd);

do_phase3(portnum, p1insur);
